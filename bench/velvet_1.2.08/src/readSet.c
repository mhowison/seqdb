...
static void readSeqDBFile(SequencesWriter *seqWriteInfo, char *filename, Category cat, IDnum *sequenceIndex, ReferenceCoordinateTable * refCoords)
{
	size_t seqCapacity = 0;
	char *seq = NULL;
	unsigned long recno, readCount;
	...
	if (cat == REFERENCE) {
		velvetLog("SeqDB file %s cannot contain reference sequences.\n", filename);
		velvetLog("Please check the command line.\n");
#ifdef DEBUG 
		abort();
#endif 
	...
	if (strcmp(filename, "-") != 0) {
		file = seqdb_open(filename);
	}
	else {
		velvetLog("Cannot open SeqDB from stdin.\n");
		exit(1);
	}

	//if (file != NULL)
		velvetLog("Reading SeqDB file %s\n", filename);
	//else
	//	exitErrorf(EXIT_FAILURE, true, "Could not open %s", filename);
	...
	readCount = 0;
	recno = 0;
	while (seqdb_next_record(file)) {
		recno++;
		size_t readLength = seqdb_seq_len(file);
		if (seqCapacity < readLength + 1) {
			seqCapacity = readLength + 1;
			seq = reallocOrExit(seq, seqCapacity, char);
		}
		seqdb_seq(file, seq);
		seqdb_id(file, qname);
	...
	for (argIndex = 1; argIndex < argc; argIndex++) {
		if (argv[argIndex][0] == '-' && strlen(argv[argIndex]) > 1) {
			...
			if (strcmp(argv[argIndex], "-fastq") == 0)
				...
				filetype = BAM;
			else if (strcmp(argv[argIndex], "-seqdb") == 0)
				filetype = SEQDB;
		...
		case BAM:
			...
			break;
		case SEQDB:
			readSeqDBFile(seqWriteInfo, argv[argIndex], cat, &sequenceIndex, refCoords);
			break;
		default:
